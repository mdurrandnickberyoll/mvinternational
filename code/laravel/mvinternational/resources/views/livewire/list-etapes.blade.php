        ...
        <div class="col-lg-12 col-xl-12">
            ...
            <div class="card">
                <div class="card-header border-bottom-0 p-4">
                    <h2 class="card-title ">{{ $workflow->libelle }}    </h2>
                     <div class="page-options d-flex float-end">
                        <a class="btn btn-primary mb-4" href="{{ route('etape',[$workflow->id]) }}"> Ajouter une ligne</a>
                    </div>
                </div>
                                ...
                                @if (!empty($datas))
                                    @foreach ($datas as $item)
                                        <tr>
                                            ...
                                            <td class="text-nowrap align-middle">{{ $item->role->libelle }}</td>
                                            <td class="text-nowrap align-middle">{{ $item->statut_initial($item->statut_debut)->libelle }}</td>
                                            <td class="text-nowrap align-middle">{{ $item->statut_final($item->statut_fin)->libelle }}</td>
                                            <td class="text-nowrap align-middle">{{ $item->sequence }}</td>
                                            <td class="text-center align-middle">
                                                <div class="btn-group align-top">
                                                    <a class="btn btn-sm btn-primary badge" href="{{ route('etape_edit', [$item->id]) }}" type="button"><i class="fa fa-edit"></i></a>
                                                    <a class="btn btn-sm btn-primary badge" href="{{ route('etape_del', [$item->id, 'supp']) }}" type="button"><i class="fa fa-trash"></i></a>
                                                </div>
                                            </td> 
                                        </tr>
                                    @endforeach
