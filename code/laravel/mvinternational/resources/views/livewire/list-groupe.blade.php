            ...
            <div class="card">
                <div class="card-header border-bottom-0 p-4">
                    ...
                    <div class="page-options d-flex float-end">
                        <a class="btn btn-primary mb-4" href="{{ route('groupe') }}"> Ajouter une ligne</a>
                    </div>
                </div>
                        ...
                        <table id="file-datatable" class="table border-top table-bordered mb-0">
                            <thead>
                                <tr>
                                    <th>Libellé</th>
                                    <th>Description</th>
                                    <th>Jour de prière communautaire</th>
                                    <th>Heure de prière communautaire</th>
                                ...
                                @if (!empty($datas))
                                    @foreach ($datas as $item)
                                        <tr>
                                            ...
                                            <td class="text-nowrap align-middle">{{ !empty($item->jour->libelle)  ? $item->jour->libelle : '' }}</td>
                                            <td class="text-nowrap align-middle">{{ !empty($item->heure->libelle) ? $item->jour->libelle : '' }}</td>
                                            <td class="text-center align-middle">
                                                <div class="btn-group align-top">
                                                    <a class="btn btn-sm btn-primary badge"
                                                        href="{{ route('groupe_edit', [$item->id]) }}"
                                                        type="button"><i class="fa fa-edit"></i></a>
                                                    <a class="btn btn-sm btn-primary badge"
                                                        href="{{ route('groupe_del', [$item->id, 'supp']) }}"
