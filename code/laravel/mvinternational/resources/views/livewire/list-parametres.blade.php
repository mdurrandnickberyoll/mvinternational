            ...
            <div class="card">
                <div class="card-header border-bottom-0 p-4">
                    ...
                    <div class="page-options d-flex float-end">
                        <a class="btn btn-primary mb-4" href="{{ route('parametre') }}"> Ajouter une ligne</a>
                    </div>
                </div>
                                ...
                                @if (!empty($datas))
                                    @foreach ($datas as $item)
                                        <tr>
                                            ...
                                            <td class="text-nowrap align-middle">{{ $item->code }}</td>
                                            <td class="text-nowrap align-middle">{{ $item->valeur }}
                                            </td>
                                            <td class="text-nowrap align-middle">{{ $item->description }}
                                            </td>
                                            <td class="text-center align-middle">
                                                <div class="btn-group align-top">
                                                    <a class="btn btn-sm btn-primary badge"
                                                        href="{{ route('parametre_edit', [$item->id]) }}"
                                                        type="button"><i class="fa fa-edit"></i></a>
                                                    <a class="btn btn-sm btn-primary badge"
                                                        href="{{ route('parametre_del', [$item->id, 'supp']) }}"
