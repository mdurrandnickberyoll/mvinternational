            ...
            <div class="card">
                <div class="card-header border-bottom-0 p-4">
                    <h2 class="card-title ">   </h2>
                     ...
                     <a class="btn btn-primary mb-4" href="{{ route('groupe_batisseur_jour_heure',[$groupe_batisseur_jour->id]) }}"> Ajouter une ligne</a> 
                     </div>
                </div>
                                ...
                                @if (!empty($datas))
                                    @foreach ($datas as $item)
                                        <tr>
                                            ...
                                            <td class="text-nowrap align-middle">{{ $item->heure->libelle }}</td>
                                            <td class="text-nowrap align-middle">{{ $item->created_at->format('d-m-Y')}}</td>
                                             
                                            <td class="text-center align-middle">
                                                <div class="btn-group align-top">
                                                    <a class="btn btn-sm btn-primary badge" href="{{ route('groupe_batisseur_jour_heure_edit', [$item->id]) }}" type="button"><i class="fa fa-edit"></i></a>
                                                    <a class="btn btn-sm btn-primary badge" href="{{ route('groupe_batisseur_jour_heure_del', [$item->id, 'supp']) }}" type="button"><i class="fa fa-trash"></i></a>
                                                </div>
                                            </td> 
                                        </tr>
                                    @endforeach
